...
class ACBlacklistAddSubCommand extends BaseSubCommand
{
    /**
     * @return void
    ...
    public function onRun(CommandSender $sender, string $aliasUsed, array $args): void
    {
        ...
        if($session->isBlacklist()) {
            $sender->sendMessage(LanguageHandler::getInstance()->translate("already_blacklisted", [$player->getName()]));
            return;
        }

        $session->addBlacklist();
        $sender->sendMessage(LanguageHandler::getInstance()->translate("success_blacklisted", [$player->getName()]));
    }
}