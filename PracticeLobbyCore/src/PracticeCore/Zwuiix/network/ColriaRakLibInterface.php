...
final class ColriaRakLibInterface extends RakLibInterface{
	...
	public function __construct(Server $server, ProtocolAcceptor $acceptor, string $ip, int $port, bool $ipV6){
		...
		ReflectionUtils::setProperty(RakLibInterface::class, $this, "rakLib", new ColriaRakLibServer(
			$server->getLogger(),
			$mainToThreadBuffer,
			$threadToMainBuffer,
	...
	public function onClientConnect(int $sessionId, string $address, int $port, int $clientID) : void{
		...
		$session = new ColriaNetworkSession(
			$this->_server,
			$this,
