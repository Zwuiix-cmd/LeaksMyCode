...
class ColriaPacketBroadcaster implements PacketBroadcaster{
	...
	/**
	 * @param ColriaNetworkSession[]  $recipients
	...
	public function broadcastPackets(array $recipients, array $packets) : void{
		...
		foreach($packets as $packet){
			...
			if($pk !== null){
				$translatedBuffers[] = ColriaPacketBatch::rawFromPackets($this->translator->getPacketSerializerFactory(), $pk)->getBuffer();
			}
		}
		...
		$packetBuffers = [];
		$compressors = [];
		/** @var ColriaNetworkSession[][][] $targetMap */
		$targetMap = [];
