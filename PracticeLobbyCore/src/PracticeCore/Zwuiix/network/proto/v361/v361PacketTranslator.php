...
class v361PacketTranslator extends PacketTranslator{
	...
	public function __construct(Server $server){
		...
		$this->itemMapping = json_decode(file_get_contents(Path::join(MultiProtocol::$resourcePath, "v361", "item_id_map.json")), true);

		$itemDeserializerFunc = Closure::fromCallable([Item::class, "jsonDeserialize"]);
		$recipes = json_decode(file_get_contents(Path::join(MultiProtocol::$resourcePath, "v361", "recipes.json")), true);
	...
	public function handleOutgoing(ClientboundPacket $pk) : ?ClientboundPacket{
        ...
        if($pk instanceof InventoryTransactionPacket) return v361InventoryTransactionPacket::create($pk->requestId, $pk->requestChangedSlots, $pk->trData);
	...
	public function handleInGame(NetworkSession $session) : ?InGamePacketHandler{
        return $session instanceof ColriaNetworkSession ? new v361InGamePacketHandler($session->getPlayer(), $session, $session->getInvManager()) : null;
	}
    ...
    /**
     * @return v361RuntimeBlockMapping
     */
    public function getBlockMapping(): v361RuntimeBlockMapping
    {
