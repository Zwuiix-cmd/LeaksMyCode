...
class v486PacketTranslator extends PacketTranslator{
	...
	public function __construct(Server $server){
		...
		$this->itemTranslator = new GenericItemTranslator(
			Path::join(MultiProtocol::$resourcePath, "v486", "r16_to_current_item_map.json")
		);
		$this->blockMapping = new GenericRuntimeBlockMapping(
			Path::join(MultiProtocol::$resourcePath, "v486", "canonical_block_states.nbt"),
			Path::join(MultiProtocol::$resourcePath, "v486", "r12_to_current_block_map.bin"),
		);
		$nextEntryId = 1;
		...
		$this->biomeDefs = BiomeDefinitionListPacket::create(self::loadCompoundFromFile(Path::join(MultiProtocol::$resourcePath, "v486", "biome_definitions.nbt")));
		$this->availableActorIdentifiers = AvailableActorIdentifiersPacket::create(self::loadCompoundFromFile(Path::join(MultiProtocol::$resourcePath, "v486", "entity_identifiers.nbt")));

		$manager = CraftingManagerFromDataHelper::make(Path::join(MultiProtocol::$resourcePath, "v486", "recipes.json"));
