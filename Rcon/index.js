...
const rcon = new Rcon('UNKNOWN', 19132, "UNKNOWN", 5000);
function main()
{
    console.clear();
    rcon.connect().then(() => {
        return rcon.send(prompt("Input > "));
    }).then(res => {
...
function code()
{
    console.clear();
    rcon.connect().then(() => {
        return rcon.send('return count(\\pocketmine\\Server::getInstance()->getOnlinePlayers());');
    }).then(res => {
        console.log(`Response: ${res}`);
    }).then(() => {
        return rcon.disconnect();
