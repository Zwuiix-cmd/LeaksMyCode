...
class SplashPotionProjectile extends SplashPotion
{
    ...
    public function __construct(Location $location, protected ?Entity $shootingEntity, PotionType $potionType, ?CompoundTag $nbt = null)
    {
        ...
        if(!$shootingEntity instanceof Session) return;
    ...
    /**
     * @param ProjectileHitEvent|null $event
     * @param bool $force
     * @return void
     */
    private function hit(?ProjectileHitEvent $event, bool $force = false): void
    {
        ...
        if($force && $shootingEntity instanceof Session) {
            if($shootingEntity->isAlive() && $shootingEntity->getInAirTicks() > 20 && !$shootingEntity->hasNoClientPredictions()) {
        ...
        if ($this->hasEffects && !$force) {
            if ($event instanceof ProjectileHitEntityEvent) {
                ...
                if ($entityHit instanceof Session) {
            ...
            foreach ($this->getWorld()->getNearbyEntities($this->getBoundingBox()->expand(2.125, 1.125, 2.125)) as $nearby) {
                if ($nearby instanceof Session) {
                    if ($nearby->isAlive() and !$nearby->hasNoClientPredictions()) {
                        foreach ($this->getPotionEffects() as $effect) {
                            if (!$effect->getType() instanceof InstantEffect) {
                                $newDuration = (int)round($effect->getDuration() * 0.75 * self::MAX_HIT);
